...
Primary objective of this research is to develop a GPT-like interactive language model for the Sinhala language, capable of enhancing written content according to specific requirements, such as eliminating grammatical errors and improving readability.
...
Roberta (a transformer-based model) is fine-tuned to extract data from the user prompts. This is useful to identify whether any information is missing in the user-given prompt, so that we can ask the user for that information.

The training dataset is curated from a set of 700 official letters into sections using a pattern-based approach. Common phrases and letter structures were used in this.

Figure \ref{fig:4.4} shows a sample of patterns used to structure the letters.

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{images/letter_patterns.png}
    \caption{Lette structuring patterns}
    \label{fig:4.4}
\end{figure}

The structure of the curated data is shown below.
...
The Named Entity Recognition (NER) module plays a crucial role in refining the quality and neutrality of the data embedded into the FAISS index. In this pipeline, the primary goal of the NER step is to eliminate personal or location-specific entities—such as names, institutions, or places—from the body of Sinhala letters. This is essential because retaining such named entities can introduce unintentional bias into the retrieval process, leading to inaccurate or overly specific outputs when users query the system. By removing these entities, the model ensures that the embeddings stored in the index reflect only the structural and contextual essence of the letter—such as whether it is a request, apology, or invitation. The NER module is implemented through a fine-tuning process using a multilingual base model (such as XLM-Roberta), which is enhanced with both rule-based and spaCy-powered components. The code supports configurable training options including batch size, learning rate, and epoch count, and offers flexibility to skip dataset preparation if previously processed. Once fine-tuned, this model effectively identifies and redacts sensitive or domain-specific tokens, making the data suitable for general-purpose use in a generation pipeline. Ultimately, the NER process is critical for maintaining a clean, generalized, and context-focused dataset, which strengthens the reliability and relevance of the entire Sinhala letter generation system. Folloeing are the specifications used in the fine tuning process.
