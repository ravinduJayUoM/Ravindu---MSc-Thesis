...
This research addresses a critical gap in Sinhala language processing. The purpose of this project is to create an interactive model that enhances Sinhala text based on specific requirements. While various languages benefit from advanced NLP tools, Sinhala lacks similar resources. The primary objective is to develop a user-friendly, interactive language model enabling users to refine Sinhala text by correcting grammatical and other errors and improving readability. This system does not generate new content but focuses on enhancing input text. By doing so, it helps people, especially government officials, to elevate the quality of their written communication.
...
The primary objective of this research is to enhance the quality of formal Sinhala letter generation through a Retrieval-Augmented Generation approach that combines semantic search with multilingual language models. The specific objectives are:

\begin{itemize}
    \item \textbf{Develop a two-stage retrieval architecture} combining FAISS-based dense vector search with cross-encoder re-ranking to improve precision in retrieving relevant Sinhala letter examples, addressing the limitations of single-stage embedding similarity approaches.
...
\begin{itemize}
    \item  Writing formal letters such as requests, replies, invitations, and official communications.
    \item Information extraction from user prompts and interacting with the user to obtain missing information if needed. 
    ...
    \item Using FAISS to store the example data in Sinhala and retrieve examples based on user prompts.
    \item Qualitative evaluation of the generated outputs  using human assessment. 
...
\begin{itemize}
    \item Generation of text other than formal letters. Notices, News-paper articles, summaries, essays, meeting minutes, posters, etc. are not covered in this system.
    ...
    \item Error correction and grammar checking of the generated text is not addressed.
    \item Expansion to other languages.
\end{itemize}
...
\begin{enumerate}
    \item Request Letters (ඉල්ලීම් ලිපි) - Used to formally request information, permissions, resources, or actions from authorities or institutions.
    \item Invitation Letters (ආරාධනා ලිපි) - Formal invitations to events, ceremonies, or meetings.
    \item Thank You Letters (ස්තූති ලිපි) - Expressing formal gratitude for assistance, support, or participation.
    \item Recommendation Letters (නිර්දේශ ලිපි) - Providing formal recommendations for individuals for employment, education, or other purposes.
    \item Inquiry Letters (විචාරණ ලිපි) - Seeking information or clarification on specific matters.
    \item Transfer Letters (මාරු ලිපි) - Requesting or notifying transfers of position, location, or responsibility.
    \item Leave Letters (නිවාඩු ලිපි) - Formally requesting time off from work or educational institutions.
    \item Complaint Letters (පැමිණිලි ලිපි) - Lodging formal complaints about services, conduct, or situations.
    \item Apology Letters (සමාව ඉල්ලීමේ ලිපි) - Offering formal apologies for mistakes, delays, or misconduct.
\end{enumerate}
