...
\usepackage{tabularx}
\usepackage{booktabs}
%\renewcommand\tabularxcolumn\citet{1}{m{#1}}% for vertical centering text in X column
...
This study focuses on a significant gap in Sinhala language processing. Its goal is to design an interactive model that supports Sinhala letter writing based on users’ specific needs. Although many other languages already benefit from advanced NLP tools, comparable resources for Sinhala remain limited. The main aim of this study is to design an efficient pipeline for generating formal Sinhala letters that improves linguistic quality, grammatical accuracy, and overall clarity. The research centers on the use of Retrieval-Augmented Generation (RAG), prompt engineering strategies, and additional Natural Language Processing (NLP) techniques to methodically enhance both the creation and refinement of Sinhala letters.. In doing so, it contributes to the advancement of Sinhala-focused NLP tools and offers individuals a practical resource to generate context rich quality letters.


\section{Research Questions}
\subsection{Main Research Question}
How to improve the quality of formal Sinhala letter writing in Large Language Models using a Retrieval-Augmented Generation approach?

\subsection{Sub Research Questions}
\begin{enumerate}
    \item What are the limitations in current Large language Models in generating formal Sinhala letters?
...
\begin{itemize}
    \item \textbf{Design a two-stage retrieval pipeline} that integrates FAISS-based dense vector search with cross-encoder reranking to enhance the precision of retrieving relevant Sinhala letter examples, overcoming the drawbacks of single-stage, embedding-only similarity methods.
    
    \item \textbf{Develop automated information extraction} by fine-tuning XLM-RoBERTa transformers to convert informal Sinhala user prompts into structured fields (letter type, recipient, subject, purpose), enabling template-driven letter generation with minimal effort from the user.
    
    \item \textbf{Build a curated Sinhala letter repository} enriched with structured metadata (category, degree of formality, document type) and robust privacy-preserving anonymization, to function as a reusable knowledge base for formal writing tasks across varied use cases.
    
    \item \textbf{Compare the RAG-based system with baseline LLM-only generation} via a systematic evaluation that measures grammatical accuracy, structural consistency, contextual relevance, and appropriateness of formality, all judged by human experts.
    
    \item \textbf{Create a continuous refinement workflow} in which high-quality, user-generated letters are fed back into the knowledge base, forming a self-improving feedback loop that incrementally broadens coverage and enhances overall quality.
    
    \item \textbf{Analyze the performance of multilingual models} (LaBSE, XLM-RoBERTa, Aya 8B) on low-resource Sinhala tasks, providing empirical findings on cross-lingual transfer effectiveness for under-resourced languages in specialized application domains.
    
    \item \textbf{Deliver a reproducible open-source toolkit} that can be deployed on modest hardware with local inference (Ollama), removing reliance on commercial APIs and promoting accessible research and development for low-resource language communities.
\end{itemize}
...
\section{Scope}
This research targets a RAG-based method for generating formal Sinhala letters in response to user prompts. Appropriate models have been employed for transforming user inputs, embedding the prompts, and generating text. The sections below explain what is included within the scope of this research and what is excluded.

\subsection{In Scope}

\begin{itemize}
    \item The research primarily concentrates on formal letters used for official requests, invitations, and formal communications.
    \item Extracting information from user prompts with a transformer model and interacting with the user to gather any missing details required to generate the letters.
    \item Designing a RAG-based framework for generating formal letters in the Sinhala language.
    \item Employing LaBSE to generate text embeddings.
    \item Using FAISS to store example Sinhala data and to perform semantic retrieval of examples relevant to the user prompts.
    \item Conducting qualitative evaluation of the generated letters for user prompts through human assessment.
    \item Merging and preprocessing two datasets to construct a more accurate dataset to be used as the retrieval knowledge base.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Deployment feasibility:} With 4.8GB memory footprint when 4-bit quantized, Aya 8B can be deployed on modest hardware (24GB RAM, 4 CPU cores) using Ollama, making it accessible for research and practical deployment without expensive cloud infrastructure. This aligns with the research objective of providing reproducible frameworks for low-resource language communities \cite{35}.
    
    \item \textbf{Generation quality:} In pilot tests, Aya 8B demonstrated superior ability to maintain formal Sinhala register (ලිඛිත භාෂාව), correctly apply honorific verb forms based on context, and structure letters according to conventional Sinhala formal letter templates – capabilities that general-purpose models like GPT-3.5 struggled with despite their larger scale.
    
    \item \textbf{Context window:} Aya 8B supports an 8192-token context window, sufficient for incorporating multiple retrieved letter examples in the RAG pipeline along with extraction results and generation instructions\cite{35}.
    
    \item \textbf{Open weights and local inference:} Unlike proprietary models, Aya 8B's weights are publicly available, enabling full local deployment, reproducibility, and avoiding ongoing API costs or data privacy concerns associated with cloud-based LLMs.
\end{itemize}

The selection of Aya 8B represents a strategic balance between model capability, resource requirements, and task-specific performance for Sinhala. While larger models like GPT-4 may demonstrate superior performance on some English benchmarks, Aya 8B's focused training on low-resource languages and its demonstrated effectiveness in Sinhala information extraction and formal text generation make it the optimal choice for this research. This decision aligns with the broader research goal of developing accessible, reproducible NLP solutions for low-resource languages that do not require expensive computational infrastructure or proprietary API access.
...
FAISS (Facebook AI Similarity Search) is a library developed by Meta AI Research for efficient similarity search and clustering of dense vectors \citet{johnson2019billion}. Written in C++ with complete Python wrappers, FAISS implements optimized algorithms for searching in large-scale vector datasets, making it ideal for retrieval-augmented generation systems.
...
XLM-RoBERTa-base was selected as the foundation model for NER tasks due to its strong cross-lingual transfer capabilities. Unlike English-centric BERT, XLM-RoBERTa was pretrained on 2.5TB of CommonCrawl data across 100 languages, including Sinhala \citet{conneau2019unsupervised}. This multilingual pretraining provides contextual representations that capture Sinhala linguistic structure without requiring language-specific model development.
...
Consists of REST APIs to interact with the pipeline. Table 4.1 shows the endpoints with a description of how the backend APIs behave. More details on this are on the API spec on GitHub repository \citet{29}.
...
\begin{tabular}{|l|c|c|}
\hline
\textbf{Metric} & \textbf{NER-Based} & \textbf{LLM-Based (Aya 8B)} \\ \hline
...
\begin{itemize}
    ...
    \item \textbf{Resource Intensive:} Requires 4.8GB GPU memory
    \item \textbf{Dependency:} Relies on availability of multilingual LLM
\end{itemize}

\textbf{Implementation Decision}

For this research, the \textbf{LLM-based extraction using Aya 8B} was selected as the primary method based on:
