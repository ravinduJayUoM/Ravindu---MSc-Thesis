...
% Set fonts that support Sinhala Unicode
\setmainfont{FreeSerif}
\setsansfont{FreeSans}
% Use a monospace font that supports Sinhala for verbatim blocks
\setmonofont{Noto Sans Mono}[
    Scale=0.9,
    % Fallback to Noto Sans Sinhala for Sinhala characters
    UprightFont=*,
    BoldFont=*-Bold,
]
...
% Make verbatim use a font that supports Sinhala
\usepackage{fancyvrb}
\fvset{fontfamily=\ttfamily, fontsize=\small}
% Redefine verbatim to support Sinhala
\DefineVerbatimEnvironment{verbatim}{Verbatim}{fontfamily=\sinhalafont}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth,keepaspectratio]{images/high_level_architecture.png}
...
\begin{figure}
    \centering
    \includegraphics[width=1.5\linewidth]{images/letter_structure.png}
...
Extracts structured fields from informal user prompts to populate letter templates. For example, from the prompt \textit{"මම අසනිප් නිසා නිවාඩුවක් ඕන"}, the model identifies \texttt{letter\_type=request} and \texttt{purpose=අසනීප නිවාඩුවක්}.

\textbf{2. Privacy-Preserving NER:}
Detects and replaces personal entities (names, locations, organizations) in the corpus before embedding. This ensures FAISS retrieval focuses on structural and semantic patterns rather than entity-specific content. Original text \textit{"මම සුනිල් පෙරේරා මොරටුව විශ්වවිද්‍යාලයට..."} becomes \textit{"මම <PERSON> <ORGANIZATION>ට..."} before vectorization.
...
\begin{itemize}
    ...
    \item Keywords like \textit{"ආකෘතිය"} (template) trigger \texttt{doc\_type=structure} filter
    \item Keywords like \textit{"විධිමත්"} (formal) trigger \texttt{register=very\_formal} filter
\end{itemize}
