...
\begin{enumerate}
    \item Letter corpus is loaded from CSV containing anonymized Sinhala letters
    \item Each letter body is encoded into 768-dimensional vectors using LaBSE
    \item Vectors are stored in FAISS IndexFlatL2 structure for exact similarity search
    \item Metadata (letter type, category, formality level) is maintained in parallel arrays synchronized with vector indices
    \item Index is persisted to disk for rapid loading during inference
\end{enumerate}

\textbf{Phase 2 - Retrieval (Online):}
\begin{enumerate}
    \item User prompt is encoded into 768-dimensional query vector using the same LaBSE model
    \item FAISS performs k-nearest neighbor search using L2 distance (equivalent to cosine similarity after normalization)
    \item Top-20 candidate documents are retrieved based on embedding similarity
    \item Retrieved documents are passed to the reranking stage for precision refinement
\end{enumerate}

\textbf{Index Configuration:}

The system employs \texttt{IndexFlatL2}, which performs exhaustive exact search. While approximate methods (e.g., IVF, HNSW) offer faster search for large datasets, exact search was chosen for this research to ensure:
\begin{itemize}
    \item 100\% recall during evaluation (no retrieval errors from approximation)
    \item Deterministic results for reproducible experiments
    \item Negligible latency penalty for small corpus (<1ms search time for 150 documents)
\end{itemize}
...
\begin{itemize}
    ...
    \item Retrieves top-20 candidate documents
    \item Latency: ~5ms
\end{itemize}
...
\begin{itemize}
    ...
    \item Reranks the 20 candidates and selects top-3 most relevant
    \item Latency: ~100ms for 20 pairs
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Zero-Shot Transfer:} While not trained on Sinhala, multilingual BERT-based models demonstrate reasonable cross-lingual transfer due to shared Unicode patterns and linguistic structures
\end{itemize}
...
\begin{itemize}
    \item \textbf{Efficiency:} Avoids exhaustive cross-encoding of entire corpus (would require 120 forward passes per query)
    \item \textbf{Precision:} Improves Precision@3 by 19\% compared to FAISS-only retrieval
    \item \textbf{Scalability:} Total latency (~105ms) remains acceptable for interactive use while maintaining high retrieval quality
\end{itemize}
...
\begin{itemize}
    ...
    \item Enhanced: FAISS + Cross-encoder reranking (refined top-3 from 20 candidates)
\end{itemize}

Experimental results demonstrate that reranking significantly improves the semantic relevance of retrieved examples, directly contributing to higher-quality letter generation by providing more contextually appropriate templates to the LLM.

\subsection{Transformer Model}

The system employs transformer-based Named Entity Recognition (NER) for two critical functions: (1) extracting structured information from user prompts, and (2) anonymizing the letter corpus before embedding to remove personal identifiers that could bias retrieval.
...
The base XLM-RoBERTa model is fine-tuned on Sinhala letter data for token-level classification using the BIO (Begin-Inside-Outside) tagging scheme. The model learns to identify six entity types critical for letter generation:

\begin{itemize}
    \item \textbf{LETTER\_TYPE:} Category classification (application, request, complaint)
    ...
    \item \textbf{DATE:} Time-related information (deadlines, dates)
\end{itemize}
...
\textbf{2. Privacy-Preserving NER:}
...
Evaluation on held-out test data demonstrates 88.4\% entity-level F1 score, significantly outperforming regex baselines (45.2\%) and validating the transformer approach for Sinhala NER tasks.
...
The system combines vector search with metadata constraints through a two-step process:

\begin{enumerate}
    \item \textbf{Pre-Retrieval Filtering:} If metadata constraints are specified (e.g., \texttt{letter\_category="request"}), the system creates a filtered subset of the corpus before vector search
    \item \textbf{Vector Search:} FAISS search executes on the filtered subset or full corpus
    \item \textbf{Post-Retrieval Augmentation:} Retrieved results include both similarity scores and metadata fields for downstream processing
\end{enumerate}
...
The generation pipeline employs retrieval-augmented generation (RAG), where the most similar letter structures are retrieved from the FAISS-indexed dataset. After retrieval, the pipeline utilizes prompt engineering techniques—discussed in detail in the subsequent section—to construct an enhanced prompt that is passed to the configured LLM for generation. The pipeline is designed to be model-agnostic: the LLM used at this stage is determined entirely by configuration, enabling any supported provider (such as Google Gemini or Ollama-hosted models) to be used interchangeably. Following generation, the output is evaluated by users, and the prompt is refined based on their feedback. Acceptable outputs undergo named entity recognition (NER) processing to remove entities before being re-embedded and added back to the FAISS index, continuously enhancing the dataset's quality and breadth.

\section{Hypothesis Testing Framework}

To test the hypotheses stated in Chapter 1 (Section 1.4), this research employs a rigorous comparative evaluation design that directly addresses the claims made in both the null hypothesis (H0) and alternative hypothesis (H1). The testing framework is designed to provide empirical evidence regarding whether RAG significantly improves formal Sinhala letter generation compared to standard LLM approaches.

\subsection{Experimental Design}

The hypothesis testing follows a between-subjects comparative design with two experimental conditions:

\textbf{Baseline Condition (Control):}
\begin{itemize}
    \item \textbf{Model:} Multilingual LLM without retrieval augmentation (using the same configured LLM as the experimental condition, ensuring a fair comparison)
    \item \textbf{Input:} User prompt is directly passed to the LLM with minimal prompt engineering
    \item \textbf{Context:} No retrieved letter examples provided; model relies solely on its pretrained knowledge of Sinhala formal writing conventions
    \item \textbf{Purpose:} Represents the current state-of-the-art for multilingual LLM performance on low-resource formal text generation
\end{itemize}

\textbf{Experimental Condition (RAG):}
\begin{itemize}
    \item \textbf{Model:} Same multilingual LLM with the full RAG pipeline applied
    \item \textbf{Input:} User prompt undergoes information extraction, semantic retrieval (FAISS + cross-encoder reranking), and augmented prompt construction
    \item \textbf{Context:} Top-3 most relevant letter examples retrieved from knowledge base and included in generation context
    \item \textbf{Purpose:} Tests whether the retrieval-augmented approach compensates for limited Sinhala training data in the underlying model
\end{itemize}

\subsection{Evaluation Metrics}

The evaluation metrics are explicitly aligned with the hypothesis variables to ensure each claim in H0 and H1 can be empirically tested:
...
\begin{itemize}
    \item Verb form correctness (tense, aspect, honorific level)
    \item Subject-verb agreement
    \item Noun case marking accuracy
    \item Pronoun usage appropriateness (honorific vs. informal)
    \item Sentence structure validity
\end{itemize}

\textbf{Rating Scale:} 1 (Multiple critical errors) to 5 (Grammatically flawless)

\subsubsection{Structural Correctness}
\textbf{Definition:} Adherence to conventional Sinhala formal letter structure including proper sequencing of components (salutation, subject line, body, closing).

\textbf{Assessment Criteria:}
\begin{itemize}
    \item Presence of all required structural components
    \item Correct ordering of letter sections
    \item Appropriate use of formal opening and closing phrases
    \item Logical paragraph organization
    \item Proper formatting conventions
\end{itemize}
...
\textbf{Assessment Criteria:}
\begin{itemize}
    \item Content alignment with stated purpose
    \item Appropriate information inclusion/omission
    \item Relevance to specified recipient and context
    \item Semantic coherence throughout the letter
    \item Appropriate tone for the letter category
\end{itemize}
...
\textbf{Assessment Criteria:}
\begin{itemize}
    \item Sentence flow and readability
    \item Lexical appropriateness (formal vs. colloquial vocabulary)
    \item Idiomaticity of expressions
    \item Consistent register maintenance
    \item Absence of code-mixing or translationese
\end{itemize}
...
\begin{itemize}
    \item All nine letter categories (request, application, complaint, invitation, appreciation, recommendation, inquiry, transfer, leave)
    \item Varying complexity levels (simple single-purpose vs. complex multi-requirement letters)
    \item Different recipient types (government officials, educational institutions, private organizations)
    \item Diverse formality requirements (formal vs. very formal contexts)
\end{itemize}

\subsubsection{Generation Procedure}
For each test prompt:
\begin{enumerate}
    \item Generate letter using the configured LLM directly, without retrieval (control condition)
    \item Generate letter using the full RAG pipeline with the same LLM (experimental condition)
    \item Randomize output order to prevent evaluation bias
    \item Present both outputs to evaluators without revealing which is RAG vs. baseline
\end{enumerate}

\subsubsection{Expert Evaluation Panel}
...
\begin{itemize}
    ...
    \item Professional experience in formal Sinhala writing (minimum 5 years)
    \item Diverse backgrounds: educators, government administrators, linguists, professional writers
    \item No prior knowledge of the research hypotheses to ensure unbiased assessment
\end{itemize}

\subsubsection{Rating Procedure}
Each evaluator independently:
\begin{enumerate}
    \item Reviews both generated letters for each prompt (baseline and RAG, order randomized)
    \item Rates each letter on all four metrics using the 5-point scales defined above
    \item Provides optional qualitative comments explaining ratings
    \item Completes evaluation without time pressure to ensure careful assessment
\end{enumerate}
...
\subsubsection{Primary Analysis: Paired t-test}
For each metric, a paired t-test compares baseline vs. RAG scores across all test prompts:
\begin{itemize}
    ...
    \item \textbf{Decision rule:} If p < 0.05 for all four metrics, reject H0
\end{itemize}

\subsubsection{Inter-Rater Reliability}
Krippendorff's Alpha is computed to assess agreement among the five evaluators:
\begin{itemize}
    \item α > 0.80: High agreement (evaluation protocol is reliable)
    \item 0.67 < α < 0.80: Moderate agreement (tentative conclusions)
    \item α < 0.67: Low agreement (evaluation may be unreliable)
\end{itemize}

\subsubsection{Effect Size Analysis}
Cohen's d is calculated to quantify the practical significance of RAG improvements:
\begin{itemize}
    \item d > 0.8: Large effect (RAG provides substantial improvement)
    \item 0.5 < d < 0.8: Medium effect (moderate improvement)
    \item d < 0.5: Small effect (limited practical benefit)
\end{itemize}
...
\begin{enumerate}
    \item All four metrics show statistically significant improvement (p < 0.05) for RAG over baseline
    \item Inter-rater reliability (Krippendorff's Alpha) exceeds 0.67 for at least three metrics
    \item Effect sizes (Cohen's d) are at least medium (d > 0.5) for the majority of metrics
\end{enumerate}

If these criteria are met, the research provides empirical evidence that RAG significantly enhances formal Sinhala letter generation, validating the proposed approach for low-resource language tasks.
